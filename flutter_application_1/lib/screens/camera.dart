...
class DisplayPictureScreen extends StatelessWidget {
  ...
  Future<void> _convertAndSendImage(
    ...
    try {
      ...
      // Send the base64 string using your DataCubit
      await context.read<DataCubit>().postData().then(
        //add inside the post data the base64image when the api its up
        (_) {
          Navigator.push(
            context,
            MaterialPageRoute(
                builder: (context) => ValuesScreen(
                    users:
                        context.read<DataCubit>().state.clinicData!.patients)),
          );
        },
      );
    } catch (e) {
        ...
        floatingActionButton: FloatingActionButton(
          onPressed: () async {
            try {
              ...
              await _convertAndSendImage(context, imagePath);
            } catch (e) {
              print(e);
            }
